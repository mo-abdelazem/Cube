...
export default async function ProductPage({ params }: Props) {
  ...
  const { translations, variants } = product;
  const selectedVariant = variants[index];

  const productTranslation = translations?.find((trans: any) => trans.language === "en");
    ...
    <main className="max-w-[1024px] mx-auto my-8 mb-16 px-6 grid grid-cols-1 md:grid-cols-[1fr_1.2fr] gap-10 items-start">
      ...
      <section className="border border-gray-200 rounded-xl p-6 bg-white shadow-sm flex flex-col">
        <div className="relative rounded-2xl overflow-hidden border border-gray-200">
          <Image
            src={selectedVariant.images?.[0]?.url || "/images/no-image.webp"}
      ...
      <section className="flex flex-col gap-7 font-medium">
        ...
        <div className="flex gap-3 items-baseline">
          ...
          <div className="text-orange-500 font-semibold text-sm">
            {t("offPercentage", { percent: 26 })}
          </div>
          <div className="line-through text-gray-400 font-semibold text-lg">
            {t("originalPrice", { price: "59.99" })}
          </div>
        </div>
        ...
        <div className="text-sm text-gray-700 flex items-center gap-3 mt-4">
          <span className="text-gray-400">{t("sizeWeight")}</span>
          <div className="flex gap-2 ml-1">
            {variants?.map((variant: any, i: number) => (
              ...
              >
                {variant.options?.[0]?.translations?.find((opt: any) => opt.language === "en")?.value ||
                  t("defaultSize")}
              </button>
            ))}
          </div>
        ...
        <dl className="text-sm grid grid-cols-2 gap-x-6 gap-y-3 text-gray-700 font-semibold max-w-[460px]">
          <div>
            <dt>{t("type")}:</dt>
            <dd><a href="#" className="text-green-500">{t("tagOrganic")}</a></dd>
          </div>
          <div>
            <dt>{t("sku")}:</dt>
            <dd>{product.sku}</dd>
          ...
          <div>
            <dt>{t("tags")}:</dt>
            <dd>
              <a href="#" className="text-green-500">{t("tagSnack")}</a>,{" "}
              <a href="#" className="text-green-500">{t("tagOrganic")}</a>,{" "}
              <a href="#" className="text-green-500">{t("tagBrown")}</a>
            </dd>
          </div>
          <div>
            <dt>{t("life")}:</dt>
          ...
          <div>
            <dt>{t("stockLabel")}:</dt>
            <dd><a href="#" className="text-green-500">
              {t("stock", { stock: variants?.[0]?.stock || 0 })}
            </a></dd>
          </div>
        </dl>
      </section>
